...
Qed.
(* down_sources -> mem_to_resp_saves_info *)
(*  *)
(* read buffer sources down *)
...
(* Lemma tid_neq_extid_neq e1 e2: tid e1 <> tid e2 -> exact_tid e1 <> exact_tid e2. *)
  ...
  { unfold write2prop. destruct (excluded_middle_informative _).
    ...
    2: { rewrite trace_index_simpl' in *; vauto. }
      ...
      { pose proof (fpga_write_structure _ f). desc. inv H. 
        ...
        (* assert (chan = chan0) by vauto. subst chan.  *)
        red in TIDxy. simpl in TIDxy. inv TIDxy.
        exists chan0. 
...
Proof.
  ...
  rewrite META in *.
  destruct WR.
  (* META' shows equality of 2 props metas => they are the same *)
...
Proof.
  ...
  { red in FPGA_TR_WF. 
    ...
    specialize PAIR_UNIQUE with (i := trace_index r1) (j := trace_index r2) (d := def_lbl) (meta := m)
  ...
  { red in FPGA_TR_WF. 
    ...
    { subst r2. eapply trace_index_simpl. destruct H5; desf. }
    { subst r1. eapply trace_index_simpl. destruct H1; desf. }
    specialize PAIR_UNIQUE with (j := trace_index r1) (i := trace_index r2) (d := def_lbl) (meta := m)
...
Proof.
  ...
  { erewrite r_vis_cpu; eauto.
    ...
    { exfalso. unfold vis' in H.
      (* assert (trace_index r = write2prop (trace_index)) *)
      destruct (excluded_middle_informative _) as [W|W].
      {
      (* 2: { admit. apply ti_inj in H; vauto. apply (fr_irr WFG FR). } *)
      ...
      2: { unfolder'; desf. }
      (* forward eapply (fpga_write_structure (EventLab w)); vauto; ins.
      desf. *)
      ...
      (* TODO: по лемме расписать нормально что такое w2p, а не фигней страдать. *)
      (* 2: { unfold trace_labels in W_. rewrite trace_index_simpl in W_; auto. } *)
      rewrite <- H in THREAD_PROP.
      unfold trace_labels at 2 in THREAD_PROP. rewrite trace_index_simpl in THREAD_PROP; auto.
      red in THREAD_PROP. desc. vauto. } 
...
Proof.
  ...
  (* assert (is_w y) as Wy.
  { destruct CPU. destruct y; vauto. destruct e; vauto. desf. } *)
...
Proof.
  ...
  { apply seq_eqv_lr in FF.
    destruct FF as [RSP [POCHxy NR]].
    red in NR.
    destruct NR as [ACT_Y NR].
    Search vis_lt'.
    cut (is_rd_resp x \/ ~is_rd_resp x); [|unfolder'; desf; intuition ].
    red in POCHxy.
    destruct POCHxy as [SBxy SAMECH].
    destruct SAMECH as [FPGAX [FPGAY CHEQ]].
    intro R; destruct R as [RDX | NRDX].
    2: { 
      cut (is_resp y \/ ~is_resp y); [|unfolder'; desf; intuition ].
      intro R; destruct R as [RSPY | NRSPY].
      { remember fpga_resp_vis_respects_sb_notr as SUB.
      red in SUB.
      eapply (SUB x y).
      red. splits; vauto. }
      assert (Eninit y). { destruct ACT_Y; vauto. red in SBxy. destruct x, y; unfolder'; desf. }
      forward eapply (TI_LE_VIS y); vauto.
      forward eapply (TI_GE_VIS x); vauto.
      Print acyclic_union1.
      Print acyclic_union.
      Search (acyclic).
     }
    red. right. apply seq_eqv_lr. 
    red in SBxy.
    2: { unfolder'. unfold is_resp in *. destruct x; desf; intuition. }
    apply seq_eqv_lr in SBxy.
    destruct SBxy as [ACT_X [SBxy _]].
  }
Qed. *)
...
(* Lemma same_ch_eq_same_ch x y: same_ch x y -> same_chan (EventLab x) (EventLab y).
Proof.
  ins.
  destruct H as [FPGA1 [FPGA2 CH]]
  unfold same_ch, same_chan, lbl_chan_opt, chan, is_fpga, fpga_chan in *; desf. *)

(* Lemma sb_in_tb: sb G ⊆ trace_before. 
Proof.
  red; ins.
  forward eapply (proj1 tb_respects_sb x y).
  { apply seq_eqv_lr in H. desf. destruct H, H1. red in H. apply seq_eqv_lr; splits; vauto. } *)
...
(* Lemma write_fence_order_lemma i j ch meta_w meta_f (DOM_i: NOmega.lt_nat_l i (trace_length tr))
  ...
  forall w_pos_i f_pos_i f_pos_j l v head mid tail head2_h tail2_h head2 mid2 tail2,
    (STRUCTURE_I: wpool_i = head ++ (store_wp ch l v, meta_w) :: mid ++ (fence_ch_wp ch, meta_f) :: tail)
    (STRUCTURE_J: wpool_j = head2_h ++ (fence_ch_wp ch, meta_f) :: head2)
  exists w_pos_j,
    ⟪ AT_WPOS_J: Some (store_wp ch l v, meta_w) = nth_error wpool_j w_pos_j ⟫ /\
    ⟪ W_BEFORE_F_j: w_pos_j < f_pos_j ⟫.
Proof.
  induction j; [lia|].
  ins.
  ...
  { subst j.
    ...
    exists w_pos_i.
    Search nth_error.
    assert (f_pos_i = f_pos_j) by admit.
    (* show nth_error inj when appending *)
    subst f_pos_j.
    splits; auto.
    erewrite nth_error_app1; [|eapply (proj1 (nth_error_Some w_pool w_pos_i)); intro H'; rewrite H' in *; desf]; auto.
    Admitted.
   (* }
  2: { 
    remember ((IHj DOM' NEQ)) as IH'.
    clear HeqIH' IHj.
    forward eapply (TSi j) with (d := def_lbl) as TSi; auto.
    inversion TSi.

    specialize (IH' w_pos_i f_pos_i f_pos_j l v).
   } *)
  
Lemma fence_in_buffer f_req f_resp ch
  (CHAN: chan_opt f_req = Some ch)
  (EG1: EG f_req)
  (EG2: EG f_resp)
  (FREQ: is_fence_req_one f_req)
  (FRESP: is_fence_resp_one f_resp)
  (PAIR: req_resp_pair f_req f_resp)
  :
  forall j, j > trace_index f_req /\ j <= trace_index f_resp ->
  exists pos, nth_error (w_pool (states j)) pos = Some (fence_ch_wp ch, (meta f_req)).
Proof.
  Admitted.
...
Lemma fence_one_response': irreflexive (poch G ⨾ fenceonepair G ⨾ sb G ⨾ (writepair G)⁻¹ ).
  ...
  assert (exists ch, chan_opt x0 = Some ch) as [ch CHAN] by (unfolder'; desf; exists c2; desf).
  set (FIB := fence_in_buffer).
  specialize FIB with (j := S (trace_index x0)).
  forward eapply (FIB x0 x1); eauto.
  splits; try lia.
  { admit. (* fencepair in po *) }
  intro H; destruct H as [wp_pos NTH].
  forward eapply write_fence_order_lemma with (i := wp_pos).

  (* Wr_Req, Fn_req, Fn_resp, Wr_resp *)
  (* [.... write_req]*)
  (* [.... write_req, fn_req]*)
  (* [fn_req ... ]*)
  (* [.... write_req .... ]*)
  ...
  (* - Есть частичный порядок попадания в буфер
     - Если есть пара состояний и во втором
     - Индуктивно показать, что 
   *)

  (* Пусть есть два состояния

  *)

  (* count_upto (req) (index) = count_upto (resp) (index) + length buffer -- верно, но не поможет*)


  (* 1. Перед fence_req есть write_req *)
  (* 2. Перед fence_resp нет write_req *)
  (* Покажем, что верно
    
      count_upto (wr_req) i = count_upto wr_resp i + size(writepool) i
     Было  [..., write_req, fence].
     Стало [ fence ...]
     Нужно показать, что write_req куда-то делся.

     Теорема: пусть write_req и fence_req лежат рядом.

     Посмотрим на write_req_to_resp, известно что он после fence_resp.
     В этот момент write_req лежит в WP.
     
     Покажем, что во все моменты с req до resp он лежит в WP.

     Посмотрим на момент, когда его выплюнуло.

     count_upto(wr_req) i = count_upto wr_resp
   *)
(* WR_REQ <POCH> FREQ FRESP <SB> WR_RESP <IN_PAST> WR_REQ *)
}
Admitted. *)
...
Proof.
  ...
  { ins. basic_solver. }
  red. right. apply seq_eqv_lr.
  destruct FR as [[x00 [_ CO]] KEK].
  unfold transp in *.
  
  (* assert (fr G ⨾ poch G ⨾ readpair G ⊆ vis_lt').
  { 
    ...
    subst x0; subst y'.
    assert (poch G x' y). { eapply readpair_in_poch; red; ins; apply seq_eqv_lr; splits; vauto. }
